...
from preprocess_rba_ds import X_train, y_train, X_test, y_test
...
train_loader = DataLoader(LoginDataset(X_train, y_train), batch_size=64, shuffle=True) # why shuffle=true? maybe bad idea when multiple failed attempts | batch size was 64
val_loader = DataLoader(LoginDataset(X_test, y_test), batch_size=64) # batch size was 64
...
class RiskModel(nn.Module):
    def __init__(self, input_dim):
        ...
        self.net = nn.Sequential(
            # layer 1
            nn.Linear(input_dim, 16), # linear layer, 64 neurons
            nn.ReLU(), # takes input from layer and decide whether to activate the perceptron or not            
            # layer 2
            nn.Linear(16, 128), # was 64, 32
            nn.ReLU(),
            #nn.Dropout(0.3),
            # layer 3
            nn.Linear(128,32),
            nn.Sigmoid(), # try turning into softmax (output -> probability)
            # final output
...
optimizer = optim.Adam(model.parameters(), lr=1e-6) # math function defining how to calculate loss / learning rate = 0.001 (steps taken to get to a minimum) | was 1e-5
...
if __name__ == "__main__":
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            #xb = normalize(xb, p=2.0)
            #yb = normalize(yb, p=2.0)
