...
async def calculate_risk_score(
    ...
    if last_attempt and last_attempt.risk_score == 100 and not await verify_success(last_attempt.event_id):
        score = 100
    ...
    if evt.user_id and not (await db.execute(select(LoginAttempt).where(LoginAttempt.email == evt.email))).scalars().first():
        ...
        if result:
            print(f">Fuzzy match to existing emails detected. Skipping...")
        else:
            score += 15
            print(f">Attempt email is entirely new and does not match to a user. Increasing score by 15. Current score:{score}")
    ...
    if not ip_results: # scalars converts first column value (id) into ScalarResult and first() returns the first row only
        ...
        for a in ip_results:
            if a.event_id in tot_verified:
                ...
                print(f">Verified past success consistency with MFA Handler's OTP Logs for {evt.user_id}, event: {a.event_id}.")
                ip_verified.append(a.event_id)
